...
function watu_init() {
	...
	if($version != '2.68') watu_activate(true);
...
function watu_add_menu_links() {
	...
	add_menu_page(__('Watu Quizzes', 'watu'), __('Watu Quizzes', 'watu'), $view_level, 'watu_exams', 'watu_exams');
	...
	// hidden pages
	add_submenu_page(NULL, __('Add/Edit Quiz', 'watu'), __('Add/Edit Quiz', 'watu'), $view_level , 'watu_exam', 'watu_exam');
	...
	add_submenu_page(NULL, __('Exam submissions', 'watu'), __('Exam submissions', 'watu'), $view_level, 'watu_takings', 'watu_takings'); 
}
...
function watu_activate($update = false) {
	...
	update_option( "watu_version", '2.68' );
