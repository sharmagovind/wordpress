	...
	if(isset($_POST['submit']) and check_admin_referer('watu_create_edit_quiz')) {
		...
		if($_REQUEST['action'] == 'edit') { //Update goes here
			...
			$wpdb->query($wpdb->prepare("UPDATE ".WATU_EXAMS."
				...
				use_honeypot=%d, save_source_url=%d, advanced_settings = %s   
			...
			$wpdb->query($wpdb->prepare("INSERT INTO ".WATU_EXAMS." 
				...
				advanced_settings) 
				VALUES(%s, %s, %s, NOW(), %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %s, %d, %s, %d, %d, %d, %d, %d, %s)", 
				...
				$notify_email, $take_again, $times_to_take, $no_alert_unanswered, $use_honeypot, $save_source_url, $advanced_settings));
